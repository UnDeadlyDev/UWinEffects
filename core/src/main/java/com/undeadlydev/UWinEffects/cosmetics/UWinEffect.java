...
public class UWinEffect extends Cosmetic implements Purchasable {
    ...
    public UWinEffect(Main plugin, String s) {
    ...
    public ItemStack getIcon(Player p) {
        ...
        if (!p.hasPermission(autoGivePermission)) {
            if (price > 0) {
                if (plugin.getConfig().getBoolean("redPanelInLocked")) {
                    if (!sw.getWineffects().contains(id)) {
                        icon = ItemBuilder.item(XMaterial.matchXMaterial(XMaterial.RED_STAINED_GLASS_PANE.parseItem()), 1, icon.getItemMeta().getDisplayName(), icon.getItemMeta().getLore());
                    }
                }
            }
        }
        ...
        return NBTEditor.set(icon, id, NBTEditor.CUSTOM_DATA, "UWINEFFECTS", "WINEFFECT");
    }
